...
For iTerm, Terminal, and Ghostty the process additionally runs an AppleScript query to obtain the session ID, TTY, and window title — used to power the "jump back to terminal" feature. All three of those queries are **focus-scoped**: they report the frontmost window's focused terminal, current session, or selected tab, not the one running the agent. They are therefore only asked at `SessionStart` and `UserPromptSubmit`, when the agent's own tab is the one the user just interacted with; on later events (tool use, notifications) the captured title and session id are cleared instead, because asking then would stamp whichever tab is frontmost onto the session that fired. The TTY is not affected — it is read from the process, so it keeps addressing the right pane.

The `cmux` terminal uses `CMUX_SURFACE_ID` instead: an AppleScript query can only report the *focused* cmux terminal, which is not necessarily the one running the agent, so the id cmux exports into each surface is the only source that identifies this tab. It lands in `terminal_session_id`, and the app consumes it with `surface.focus` on cmux's control socket — which also switches workspace when the tab lives in another one.
